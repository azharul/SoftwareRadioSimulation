...
figure;
subplot(2,1,1),nfspec(pulse_shapedH,fsH);
...
subplot(2,1,2),nfspec(modulatedH,fsH);
...
figure;
subplot(3,1,1),nfspec(IFbandpassedH,fsH);
...
%% My dual costas
% I have used the costas loop function twice. I've fed the output of one
% costas loop to another.

% first costas
theta=costas_loop(modulated,fs,fc_prime,B);
downconv1=cos(2*pi*fc_prime*t + theta);

%second costas
theta2=costas_loop(downconv1,fs,fc_prime,B);
figure;
plot(-theta2);title('theta'); ylabel('theta');
xlabel('iteration');

downconverter = cos(2*pi*fc_prime*t + theta2);
...
% header=[header zeros(1,length(quantized_msg)-length(header))];
xc=conv(quantized_msg,fliplr(header));
xc2=conv(msg,fliplr(header));
...
offset
figure; 
subplot(2,1,1),plot(1:length(xc(offset:end)),xc(offset:end)); ylabel('conv(received,header)');
subplot(2,1,2),plot(1:length(xc2(offset:end)),xc2(offset:end)); ylabel('conv(original,header)');
...
decoded_msg = pam4_to_letters(quantized_msg(index+1:index+data_symbols))
